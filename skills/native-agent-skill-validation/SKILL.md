...
5. Before spawning, persist exact prompt and policy bytes and hashes, a direct policy diff, neutral run IDs, assignment/order, isolated workspace manifests, and the rubric in a bounded location outside the test workspace. Prefer a fresh ephemeral directory; use a durable or pre-existing location only with user authorization. Screen the retained evidence for secrets and personal data. After validation, delete only artifacts this run created in its ephemeral directory; retain and report any pre-existing or user-provided evidence unless the user explicitly authorizes deletion.
