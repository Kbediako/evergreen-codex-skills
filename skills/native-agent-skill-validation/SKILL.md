...
3. Build a realistic user-style task from raw artifacts.
4. Withhold the intended answer, suspected defect, proposed fix, and prior conclusions.
5. Keep permissions and side effects no broader than the real task requires.
...
4. Persist exact prompt bytes or hashes, skill revisions and availability, run order, session/rollout IDs, and structural invocation evidence outside the test workspace before spawning.
5. Wait for and consume each final child answer.
6. Verify cited files, outputs, commands, and mutations in the parent.
7. Treat status as lifecycle evidence only; never treat a completed status as proof the task succeeded.
8. Repeat independent isolated comparisons before promoting a behavior claim.
9. Send instruction defects back to `skill-creator` for editing, then start a fresh validation cycle.
